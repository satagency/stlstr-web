...
/** Ticketmaster partner blue (default logo fill). */
export const TICKETMASTER_LOGO_COLOR = "#014ddf";

const VIEWBOX_W = 135;
const VIEWBOX_H = 24;

type TicketmasterLogoProps = Omit<SVGProps<SVGSVGElement>, "viewBox" | "xmlns"> & {
  /** Fill color; defaults to Ticketmaster blue `#014ddf`. */
  fill?: string;
  /** When set, logo is announced to assistive tech (and `aria-hidden` is forced off). */
  title?: string;
};

/**
 * Ticketmaster wordmark (SVG path from official lockup). Use for partner / integration UI.
 * Prefer passing `title` when the logo conveys meaning; omit for decorative use.
 */
export function TicketmasterLogo({
  className,
  fill = TICKETMASTER_LOGO_COLOR,
  height = 24,
  width,
  title,
  ...
  role,
  ...rest
}: TicketmasterLogoProps) {
  const resolvedWidth =
    width ??
    (typeof height === "number" ? (height * VIEWBOX_W) / VIEWBOX_H : undefined);

  const decorative = !title;
  const resolvedAriaHidden = decorative ? (ariaHidden ?? true) : false;
